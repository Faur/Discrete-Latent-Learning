...
class Model:
    ...
    def init_network(self):
        # The model structure
        self.step_policy = self.policy(self.sess, self.X_input_step_shape, self.num_actions, reuse=False, is_training=False)
    ...
    def __set_VAE_observation_space_params(self, observation_space_params, latent_size):
        self.img_height, self.img_width, self.num_classes = observation_space_params
        self.LATENT_SIZE = latent_size

    def __set_VAE_action_space_params(self, num_actions):
        self.num_actions = num_actions
    ...
    def buildForVAE(self, observation_space_params, num_actions, latent_size):
        self.__set_VAE_observation_space_params(observation_space_params, latent_size)
        self.__set_VAE_action_space_params(num_actions)
        self.init_VAE_input()
        self.init_VAE_network()

    @staticmethod
    def policy_name_parser(policy_name):
        from A2C.models.cnn_policy import CNNPolicy
        from A2C.models.vae_policy import VAEPolicy
        policy_to_class = {'CNNPolicy': CNNPolicy, 'VAEPolicy': VAEPolicy}
